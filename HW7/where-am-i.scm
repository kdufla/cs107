...
(define (intersection-points circles)
  (if (< (length circles) 2) '()
  (append (apply append
			 (map (lambda (x)(intersect (car circles) x)) (cdr circles)))
		  (intersection-points (cdr circles)))))


(define (distance-product point points)
  (let ((modList (remove point points)))
  (apply * (map (lambda (x)(dist point x)) modList))))


(define (rate-points points)
  (map (lambda (x) (list (distance-product x points) x)) points))


(define (sort-points rPoints)
  (quicksort rPoints (lambda(a b)(< (car a) (car b)))))
...
(define (list-average coordList)
  (/ (apply + coordList) (length coordList)))

(define (average-point points)
  (let ((avg (list (list-average (map x points)) (list-average (map y points)))))
...
(define (where-am-i distances stars)
  (quicksort (map best-estimate (all-guesses distances stars)) (lambda (estOne estTwo) (< (car estOne) (car estTwo)))))
