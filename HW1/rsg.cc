...
void printText(map<string, Definition> myMap, string s){
	Definition d = myMap[s];
	Production p = d.getRandomProduction();
	for(Production::iterator c = p.begin(); c != p.end(); ++c) {
		string str=*c;
		if(str[0]=='<')printText(myMap,str);
		else cout<<str<<" ";
	}
}
