...
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *servName, const char *articleURL, hashset* stopWords, hashset* words);
...
static void getStopWords(hashset* stopWords, const char* kStopWordsFile);
...
static int wordHash(const void *s, int numBuckets);
static void wordMap(void*elemAddr,void*auxData);
static int wordCmp(const void*,const void*);
static void wordFree(void*elemAddr);

static void vecFreeArt(void*elemAddr);
int artCmp(const void *elemAddr1, const void *elemAddr2);
int artCmp2(const void *elemAddr1, const void *elemAddr2);
...
int main(int argc, char **argv)
{
  ...
  HashSetNew(&stopWords, sizeof(char*), 1009, StringHash, StringCmp, StringFree);
  HashSetNew(&words, sizeof(wordCounter), 10007, wordHash, wordCmp, wordFree);

  getStopWords(&stopWords, kStopWordsFile);
...
static void ProcessFeedFromFile(char *fileName, hashset* stopWords, hashset* words)
{
  ...
  ScanArticle(&st, (const char *)fileName, (const char *)fileName, (const char *)fileName, stopWords, words);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew doesn't open one..
  fclose(infile);
...
static void ProcessFeed(const char *remoteDocumentName, hashset* stopWords, hashset* words)
{
  ...
  if(!strncmp(filePrefix, remoteDocumentName, strlen(filePrefix))){
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(filePrefix), stopWords, words);
    return;
  }
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset* stopWords, hashset* words)
{
  ...
  switch (urlconn.responseCode) {
      ...
      case 200: printf("Scanning \"%s\" from \"http://%s\"\n", articleTitle, u.serverName);
	        STNew(&st, urlconn.dataStream, kTextDelimiters, false);
		ScanArticle(&st, articleTitle, u.serverName, articleURL, stopWords, words);
		STDispose(&st);
		break;
      case 301:
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *servName, const char *articleURL, hashset* stopWords, hashset* words)
{
  ...
  while (STNextToken(st, word, wordSize)) {
    if (strcasecmp(word, "<") == 0) {
      SkipIrrelevantContent(st); // in html-utls.h
    } else {
      RemoveEscapeCharacters(word);
      if (WordIsWellFormed(word) && !HashSetLookup(stopWords, &word)) {

        article a;
        a.url=articleURL;

        if(HashSetLookup(words, &word)!=NULL){
          wordCounter* wc=HashSetLookup(words, &word);
          vector* v=wc->articles;
          int idx=VectorSearch(v,&a, artCmp,0,0);

          if(idx>=0){
            article* artInVec = VectorNth(v,idx);
            artInVec->count++;
          }else{
            article ar;
            ar.title=strdup(articleTitle);
            ar.url=strdup(articleURL);
            ar.serverName=strdup(servName);
            ar.count=1;
            VectorAppend(wc->articles, &ar);
          }

        }else{
          wordCounter wc;
          wc.word=strdup(word);
          wc.articles=(vector*)malloc(sizeof(vector));

          VectorNew(wc.articles, sizeof(article), vecFreeArt, 8);
...
static void ProcessResponse(const char *word, hashset* stopWords, hashset* words)
{
  if (!WordIsWellFormed(word)) {
    ...
    wordCounter* x=(wordCounter*)HashSetLookup(words, &temp);
    free(temp.word);
    if (x != NULL) {
      vector* articleCounts = x->articles;
      VectorSort(articleCounts, artCmp2);
      int len = MIN(10, VectorLength(articleCounts));

      for (int i = 0; i < len; i++) {
        article *ac = VectorNth(articleCounts, i);
        char* st= ac->count==1 ? "%d.) \"%s\" [search term occurs %d time]\n" : "%d.) \"%s\" [search term occurs %d times]\n";
        printf(st, i+1, ac->title, ac->count);
        printf("   \"%s\"\n ", ac->url);
      }
    } else {
...
static int StringHash(const void *x, int numBuckets)
{
  char* s=*(char**)x;
  int i;
...
static int wordHash(const void *x, int numBuckets){
  wordCounter* y=(wordCounter*)x;
  char* s=y->word;
  return StringHash(&s,numBuckets);
}

static int wordCmp(const void*x,const void*n){
  wordCounter* y=(wordCounter*)x;
  char* s=y->word;
  wordCounter* m=(wordCounter*)n;
  char* s1=m->word;
  return StringCmp((void*)&s,(void*)&s1);
}
static void wordFree(void* x){
  wordCounter* y=(wordCounter*)x;
  vector* h=y->articles;
  VectorDispose(h);
  free(y->word);
